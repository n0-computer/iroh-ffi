...
public class Entry: EntryProtocol {
    ...
    public func timestamp() -> UInt64 {
        return try! FfiConverterUInt64.lift(
            try!
                rustCall {
                    uniffi_iroh_fn_method_entry_timestamp(self.pointer, $0)
                }
        )
    }
}
...
private var initializationResult: InitializationResult {
    ...
    if uniffi_iroh_checksum_constructor_query_single_latest_per_key_exact() != 1670 {
        return InitializationResult.apiChecksumMismatch
    }
    if uniffi_iroh_checksum_constructor_query_single_latest_per_key_prefix() != 21174 {
        return InitializationResult.apiChecksumMismatch
    }
